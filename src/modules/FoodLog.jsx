...
// reads — so everything you log here feeds the day's numbers (no engine change).
export default function FoodLog() {
  ...
  return (
    <>
      <PageHead eyebrow="Log" title="Food" sub="Log your meals — calories and protein. Describe a meal and the coach will estimate it, or enter your own." />

      <Card style={{ marginBottom: 16 }}>
        <div className="dl-nav">
          <button className="dl-arrow" onClick={() => setViewDay(v => Math.max(1, v - 1))} disabled={d <= 1} aria-label="Previous day">‹</button>
          <div className="dl-day" onClick={() => setViewDay(today)} title="Jump to today">
            <div className="dl-day-label">{label}</div>
            <div className="dl-day-date">Day {d} · {ddmm(iso)}</div>
          </div>
          <button className="dl-arrow" onClick={() => setViewDay(v => Math.min(days, v + 1))} disabled={d >= days} aria-label="Next day">›</button>
        </div>
        <div className="divider" />
        <div className="food-totals">
          <div><div className="food-big">{totalC}</div><div className="faint">/ {Math.round(planRes.calorieTarget)} kcal</div></div>
          <div><div className="food-big">{totalP}g</div><div className="faint">/ {Math.round(planRes.proteinTarget)}g protein</div></div>
        </div>
      </Card>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <div className="food-entry" style={{ marginTop: 14 }}>
          <label className="gp-f"><span>Calories</span><input type="number" placeholder="kcal" value={kcal} onChange={e => setKcal(e.target.value)} /></label>
          <label className="gp-f"><span>Protein</span><input type="number" placeholder="g" value={protein} onChange={e => setProtein(e.target.value)} /></label>
          <button className="btn primary" onClick={addMeal} disabled={!kcal && !protein && !desc.trim()}>Add</button>
        </div>
        <div className="divider" />
        <Pill tone="warn">AI estimate — not exact</Pill>
        <p className="faint" style={{ margin: '8px 0 0', fontSize: 12.5, lineHeight: 1.5 }}>
          It can't see your exact portions, so treat it as a starting point. Tweak the numbers before adding, or enter your actuals.
        </p>
      </Card>
        ...
        {meals.map((m, i) => (
          <div className="wl-set" key={i}>
            <div style={{ minWidth: 0 }}>
              <div className="wl-ex">{m.desc}</div>
              <div className="wl-meta">{m.calories || 0} kcal · {m.protein || 0}g protein</div>
            </div>
            <div className="wl-right"><button className="gp-x" onClick={() => delMeal(i)} aria-label="Delete meal">✕</button></div>
          </div>
        ))}
      </Card>
    </>
